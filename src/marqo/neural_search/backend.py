...
def add_customer_field_properties(config: Config, index_name: str,
    ...
    for field_name in customer_field_names:
        # add these as text fields to the parent doc, to make them lexical searchable
        # body["properties"][validation.validate_field_name(field_name[0])] = {
        #     "type": "text"# field_name[1]
        # }
        # new_index_properties[validation.validate_field_name(field_name[0])] = {
        #     "type": "text" # field_name[1]
        # }
        # add these fields as keywords the chunk to make them available for filtering
